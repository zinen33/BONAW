...
exec("rm -rf modules/commands/cache && rm -rf modules/commands/tad/*", (error, stdout, stderr) => {
    if (error) {
    ...
    console.log(chalk.bold.hex("#90EE90")("JONELL (JONELL CC PROJECT) ❯ ") + chalk.hex("#FF8000")("Fixed by Jonell Magallanes "));
});
...
console.log(chalk.bold.hex("#FF8000").bold("[ JONELL (JONELL CC PROJECT) ] » ") + chalk.bold.hex("#90EE90").bold("Deploying The All Variables..."));
...
try {
    var appStateFile = resolve(join(global.client.mainPath, global.config.APPSTATEPATH || "Haroldstate.json"));
...
function checkBan(checkban) {
    ...
    if (existsSync('/home/runner/.miraigban')) {
        ...
        logger(global.getText('mirai', 'banDevice'), '[ GLOBAL BAN ]'), _0x2cd8f4.on(line, _0x4244d8 => {
            ...
            else return axios.get('https://raw.githubusercontent.com/D-Jukie/gban-mirai/cf752719489810f28cb2556222f51d4cfd2e0158/listgban.json').then(_0x2f978e => {
    ...
    return axios.get('https://raw.githubusercontent.com/D-Jukie/gban-mirai/cf752719489810f28cb2556222f51d4cfd2e0158/listgban.json').then(dataGban => {
        ...
        return axios.get('https://raw.githubusercontent.com/D-Jukie/gban-mirai/d749588651d316c868840726bd9573c6c1230a32/data.json').then(json => {
...
loginApiData.setOptions(global.config.FCAOption)
    ...
    loginApiData.sendMessage("✅ | Bot Has been activated.", 
global.config.ADMINBOT[0]);         //notif if bot is kaka on palang
...
cron.schedule('0 2 1 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Its already 1:00 everyone need mo na yata mag sleep idol wala ka naman ka chat", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 2 3 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("Goodmorning everyone! Time check 3:00am Tulog kana hindi ka niya kausapin", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
...
cron.schedule('0 2 7 * * *', () => {
  loginApiData.getThreadList(30, null, ["INBOX"], (err, list) => {
    if (err) return console.log("ERR: "+err);
    list.forEach(now => (now.isGroup == true && now.threadID != list.threadID) ? loginApiData.sendMessage("its already 7am don't your breakfast!", now.threadID) : '');
  });
}, {
  scheduled: true,
  timezone: "Asia/Manila"
